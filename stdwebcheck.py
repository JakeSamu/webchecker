...
parser.add_argument('-da',                  type=str,       default=config.absolutepath,help='Absolute path')
...
parser.add_argument('--tls',                type=str2bool,  default=config.tls,         help='Only use if option -f is used. Set to False if endpoint is not using TLS.')
parser.add_argument('-r', '--redirect',     type=str2bool,  default=config.redirect,    help='Follow redirections if True.', nargs='?', const=True)
parser.add_argument('-m', '--method',       type=str,       default=config.method,      help='Only works with usage of url, defines the method. Default is GET')
parser.add_argument('--header',             type=json.loads,default=config.header,      help='Add the header of the request as a dictionary. Only compatible with -u.')
parser.add_argument('--body',               type=str,       default=config.body,        help='Define the body of the request as a string. Only compatible with -u.')
...
if config.relative: path = pathlib.Path(config.relativepath)
...
webcall.check_proxy()
if (use_file): webcall.set_file(config.file)
else: webcall.set_url(config.url)
